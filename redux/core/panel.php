<?php
/* 文件直接汉化 @ INLOJV , visit: http://www.inlojv.com */
    if ( ! defined( 'ABSPATH' ) ) {
        exit;
    }
            ...
            function notification_bar() {
                ...
                if ( isset( $this->parent->transients['last_save_mode'] ) ) {
                    ...
                    if ( $this->parent->transients['last_save_mode'] == "import" ) {
                         ...
                         */
                        echo '<div class="admin-notice notice-blue saved_notice"><strong>' . apply_filters( "redux-imported-text-{$this->parent->args['opt_name']}", __( '设置已导入!', 'redux-framework' ) ) . '</strong></div>';
                        //exit();
                         ...
                         * @param string  translated "settings imported" text
                         */
                        echo '<div class="saved_notice admin-notice notice-yellow"><strong>' . apply_filters( "redux-defaults-text-{$this->parent->args['opt_name']}", __( '全部设置已恢复默认!', 'redux-framework' ) ) . '</strong></div>';
                         ...
                         * @param string  translated "settings imported" text
                         */
                        echo '<div class="saved_notice admin-notice notice-yellow"><strong>' . apply_filters( "redux-defaults-section-text-{$this->parent->args['opt_name']}", __( '局部设置已恢复默认!', 'redux-framework' ) ) . '</strong></div>';
                         ...
                         */
                        echo '<div class="saved_notice admin-notice notice-green">' . apply_filters( "redux-saved-text-{$this->parent->args['opt_name']}", '<strong>'.__( '设置保存成功', 'redux-framework' ) ).'</strong>' . '</div>';
                    }
                 ...
                 */
                echo '<div class="redux-save-warn notice-yellow"><strong>' . apply_filters( "redux-changed-text-{$this->parent->args['opt_name']}", __( '某些设置已经改变，你最好保存一下!', 'redux-framework' ) ) . '</strong></div>';
                ...
                echo '<div class="redux-field-errors notice-red"><strong><span></span> ' . __( '发现错误!', 'redux-framework' ) . '</strong></div>';
                ...
                echo '<div class="redux-field-warnings notice-yellow"><strong><span></span> ' . __( '发现警告!', 'redux-framework' ) . '</strong></div>';
                    ...
                    if ( $developer_theme_file ) {
                        ...
                        if ( $core_version && $developer_version && version_compare( $developer_version, $core_version, '<' ) ) {
                            ...
                            <div id="message" class="error redux-message">
                                <p><?php _e( '<strong>如果你遇到功能上的问题有可能是框架文件已过期</strong> &#8211; 请确保是最新版否则请删掉它！', 'redux-framework' ); ?></p>
                            </div>
                            <?php
                            return;
                        }
